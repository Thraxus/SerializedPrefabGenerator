...
namespace Thraxus.Common.Utilities.Tools.Networking
{
	internal static class ChatHandler
	{
		...
		private const string StartSpawn = "start";
		private const string StopSpawn = "stop";

		private static readonly Dictionary<string, Action<string>> ChatAction = new Dictionary<string, Action<string>>
		{
			{HelpPrefix, PrintHelpCommands}, {StartSpawn, SessionCore.Spawner.StartSpawn}, {StopSpawn, SessionCore.Spawner.StopSpawn}
		};
		...
		private static void PrintHelpCommands(string s)
		{
			...
			Messaging.ShowLocalNotification($"'{ChatCommandPrefix} {StartSpawn}' will start the spawning routine.");
			Messaging.ShowLocalNotification($"'{ChatCommandPrefix} {StopSpawn}' will halt the spawning routine.");
		}
	}
}
